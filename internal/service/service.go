...
type Service struct {
	...
	idem                  map[string]any
	confirmedPlans        map[string]string
	Metrics               *ServiceMetrics
}

func New(st *store.Store) *Service {
	s := &Service{Store: st, Topology: domain.DefaultTopology(), Faults: map[string]*domain.Fault{}, Plans: map[string]*domain.Plan{}, Steps: map[string]*domain.Step{}, Queue: store.LoadQueue(st.Dir()), idem: map[string]any{}, confirmedPlans: map[string]string{}, Metrics: &ServiceMetrics{Started: time.Now().UTC()}}
	s.replay()
	return s
}
...
func (s *Service) ConfirmWithKey(id, key string) (*domain.Plan, error) {
	...
	if planID := s.confirmedPlans[id]; planID != "" {
		if p := s.Plans[planID]; p != nil {
			return p, nil
		}
	}
...
func (s *Service) mutateStep(id, w, a, k string, v int, res string) (*domain.Step, error) {
	...
	if a == "claim" {
		...
		if a == "complete" {
			...
			s.Reviews = append(s.Reviews, domain.ReviewRecord{ID: newID(), StepID: id, Reason: res, Worker: w, CreatedAt: time.Now().UTC()})
		}
	} else {
		return nil, domain.ErrInvalid
	}
	...
	_ = s.event("step."+string(st.Status), st.ID, st.Version, cp, k)
